...
export interface AgentTerminalCommandSpec {
  kind: 'command';
  command: string;
  args?: string[];
  startupLabel: string;
...
export interface AgentTerminalMessageSpec {
  kind: 'message';
  startupLabel: string;
  message: string;
}

export type AgentTerminalLaunchSpec =
  | AgentTerminalCommandSpec
  | AgentTerminalMessageSpec;
...
export function spawnAgent(
  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      if (code === 0) {
        ...
        resolve({
          success: false,
          message:
            stderr.trim().slice(-200) || `Process exited with code ${code}`,
        });
      }
    });
    ...
    setTimeout(() => {
      ...
      resolve({
        success: false,
        message: `Edit timed out after ${timeout / 1000} seconds`,
      });
    }, timeout);
  });
}
