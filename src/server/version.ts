...
import { stripLayrrCommitPrefix } from './commit-message.js';
...
export function preview(ws: WebSocket, hash: string, projectRoot: string) {
  try {
    if (!originalBranch) {
      try {
        originalBranch = execSync('git symbolic-ref --short HEAD', {
          cwd: projectRoot,
          encoding: 'utf-8',
        }).trim();
      } catch {
    ...
    const displayMessage = stripLayrrCommitPrefix(commitMsg);
    // Send response BEFORE checkout (checkout triggers reload which kills the WS)
    send(ws, {
      type: 'version-preview-result',
      success: true,
      hash,
      message: displayMessage,
    });
    execSync(`git checkout ${hash} --detach`, {
      cwd: projectRoot,
      stdio: 'pipe',
    });
    console.log(`  ⏪ Previewing: ${displayMessage}`);
  } catch (err: any) {
...
export function revert(ws: WebSocket, hash: string, projectRoot: string) {
  try {
    const branch =
      originalBranch ||
      execSync('git symbolic-ref --short HEAD 2>/dev/null || echo main', {
        cwd: projectRoot,
        encoding: 'utf-8',
      }).trim();
    try {
...
export function commit(ws: WebSocket, projectRoot: string) {
  try {
    execSync('git push -u origin HEAD', { cwd: projectRoot, stdio: 'pipe' });
    ...
    send(ws, { type: 'commit-result', success: false, message: err.message });
  }
}
