...
import {
  formatLayrrCommitDisplay,
  isLayrrCommitMessage,
} from './commit-message.js';
import { handleWsConnection } from './ws-handler.js';
import { editQueue } from './edit-queue.js';
import { getPtyBuffer } from './pty.js';
...
const ACCESS_TOKEN = process.env.PAIR_ACCESS_TOKEN || '';
const SHARE_PASSWORD = process.env.PAIR_SHARE_PASSWORD || '';
...
function setAuthCookie(res: ServerResponse) {
  res.setHeader(
    'Set-Cookie',
    `${COOKIE_NAME}=${ACCESS_TOKEN}; HttpOnly; Path=/; SameSite=Lax; Max-Age=86400`,
  );
}

function servePasswordGate(res: ServerResponse, error?: string) {
  const errorHtml = error
    ? `<p style="color:#fb7185;font-size:13px;margin-bottom:16px">${error}</p>`
    : '';
  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pair — Enter Password</title>
<style>
...
<div class="card">
<h1>Access Denied</h1>
<p>This editor is private. Open it from the Pair dashboard.</p>
</div>
</body></html>`;
...
export async function startProxy(
  ...
  httpServer = createServer(async (req, res) => {
    ...
    // Serve overlay JS
    if (reqPath === '/__layrr__/overlay.js') {
      try {
    ...
    if (reqPath === '/__layrr__/xterm.css') {
      try {
        const cssPath = fileURLToPath(new URL('../xterm.css', import.meta.url));
        res.writeHead(200, { 'Content-Type': 'text/css' });
        res.end(readFileSync(cssPath));
      } catch (e) {
        console.error('[layrr] Error serving xterm.css:', e);
        res.writeHead(404);
        res.end();
      }
      return;
    ...
    if (req.url === '/__layrr__/history') {
      try {
        const log = execSync('git log --format="%H|%s|%ar" -50', {
          cwd: projectRoot,
          encoding: 'utf-8',
        }).trim();
        ...
        const commits = log
          ? log.split('\n').map((line) => {
              ...
              const isPair = isLayrrCommitMessage(rawMessage);
              const message = formatLayrrCommitDisplay(rawMessage);
              return { hash, message, timeAgo, isPair };
            })
          : [];
        res.writeHead(200, {
          'Content-Type': 'application/json',
    ...
    if (req.url === '/__layrr__/pty-buffer') {
      res.writeHead(200, {
        'Content-Type': 'application/json',
        'Cache-Control': 'no-store',
      });
      res.end(JSON.stringify({ data: getPtyBuffer() }));
      return;
    }
    ...
    try {
      ...
      if (contentType.includes('text/html')) {
        ...
        const overlayVersion = Date.now();
        const overlayScript = `
          <script>window.__LAYRR_WS_PORT__ = ${proxyPort}; window.__LAYRR_AGENT__ = ${JSON.stringify(safeAgentName)}; window.__LAYRR_MODE__ = ${JSON.stringify(uiMode)};</script>
          <script src="/__layrr__/overlay.js?v=${overlayVersion}"></script>
        `;
  ...
  wss.on('connection', (ws) =>
    handleWsConnection(ws, projectRoot, agentName, uiMode),
  );
  ...
  const actualPort = await findAvailablePort(proxyPort);

  return new Promise((resolve) => {
    httpServer!.listen(actualPort, () => resolve(actualPort));
  });
}
