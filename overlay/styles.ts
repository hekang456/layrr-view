...
export function loadFonts() {
  ...
  xtermLink.href = '/__layrr__/xterm.css';
  container.appendChild(xtermLink);
...
export function injectStyles() {
  ...
  s.textContent = `
    ...
    .${L}-flyout{
      display:none;width:500px;max-height:280px;overflow-y:auto;
    ...
    .${L}-bar-confirm{top:auto;min-height:100px;border-radius:${radius.xl}}
    ...
    /* ---- Bar + Toolbar ---- */
    #${L}-bar{
      position:fixed;bottom:116px;right:16px;z-index:999999;
    ...
    .${L}-toolbar{display:flex;align-items:center;padding:5px;height:40px;gap:2px}
    .${L}-bd{
      display:flex;align-items:center;justify-content:center;
      width:20px;height:30px;cursor:grab;color:${C.textDim};
      font-size:${size.xl};transition:color .12s;flex-shrink:0;
    }
    .${L}-bd:hover{color:${C.textMuted}}
    .${L}-bd:active{cursor:grabbing}
    #${L}-bar.expanded .${L}-bd{display:none}
    ...
    .${L}-bgrv{background:transparent;color:${C.textDim}}
    .${L}-bgrv:hover{color:${C.error};transform:scale(1.02)}
    .${L}-bgrv:active{transform:scale(.97)}
    .${L}-bgcm{background:transparent;color:${C.textDim}}
    .${L}-bgcm:hover{color:${C.success};transform:scale(1.02)}
    .${L}-bgcm:active{transform:scale(.97)}

    /* ---- Edit panel ---- */
    .${L}-panel{
      display:none;width:640px;height:500px;overflow:hidden;
      border-top:1px solid ${C.panelBorder};
      border-radius:${radius.xl} ${radius.xl} 0 0;
    }
    .${L}-panel.open{display:flex;flex-direction:column}
    .${L}-panel.${L}-panel-mode-panel{
      width:320px;max-height:320px;height:auto;
    }
    .${L}-terminal-container{
      flex:1 1 auto;min-height:0;width:100%;padding:4px 8px;box-sizing:border-box;
      overflow:hidden;display:flex;flex-direction:column;background:#09090b;
    }
    .${L}-panel.${L}-panel-mode-terminal .${L}-terminal-container{
      margin-bottom:8px;
    }
    .${L}-ph{display:flex;align-items:center;justify-content:space-between;padding:8px 10px 0;user-select:none;cursor:grab;flex-shrink:0}
    ...
    .${L}-mode-tag{
      display:inline-flex;align-items:center;justify-content:center;
      margin-left:4px;padding:1px 6px;border-radius:${radius.full};
      border:1px solid rgba(96,165,250,.26);
      background:rgba(96,165,250,.12);color:rgba(191,219,254,.95);
      font-size:${size.xs};font-weight:600;letter-spacing:.04em;
    ...
    .${L}-ei{
      margin:6px 10px;padding:6px 8px;background:${C.surface};border:1px solid ${C.border};border-radius:${radius.lg};
      height:130px;overflow-y:auto;flex-shrink:0;
    }
    .${L}-panel.${L}-panel-mode-panel .${L}-ei{
      height:auto;min-height:88px;max-height:130px;
    }
    .${L}-eh{font-size:${size.md};color:${C.textDim};text-align:center;padding:8px 4px}
    .${L}-panel.${L}-panel-mode-panel .${L}-eh{
      padding:4px 4px;
    }
    ...
    .${L}-src-row{margin-top:4px;display:flex;gap:8px;align-items:flex-start}
    .${L}-sk{font-size:${size.xs};color:${C.textMuted};font-weight:600;text-transform:uppercase;letter-spacing:.06em;min-width:48px}
    .${L}-src-chip{
      font-size:${size.xs};font-family:${mono};
      padding:2px 6px;border-radius:${radius.full};border:1px solid ${C.border};
      background:${C.surface};color:${C.textMuted};
    }
    .${L}-src-chip.exact{border-color:rgba(34,197,94,.35);background:rgba(34,197,94,.08);color:rgba(34,197,94,.95)}
    .${L}-src-chip.component{border-color:rgba(96,165,250,.35);background:rgba(96,165,250,.08);color:rgba(96,165,250,.95)}
    .${L}-src-chip.file-chip{border-color:rgba(161,161,170,.35);background:rgba(161,161,170,.08);color:rgba(161,161,170,.95)}
    .${L}-src-chip.fallback{border-color:rgba(245,158,11,.35);background:rgba(245,158,11,.08);color:rgba(245,158,11,.95)}
    .${L}-src-chip.loading{border-color:rgba(161,161,170,.35);background:rgba(161,161,170,.08);color:${C.textMuted}}
    .${L}-src-path{
      font-size:${size.xs};color:${C.textDim};font-family:${mono};
      white-space:normal;overflow-wrap:anywhere;word-break:break-word;
      flex:1;min-width:0;
    }
    .${L}-precision-warn{
      margin-top:6px;padding:5px 8px;border-radius:${radius.md};
      border:1px solid rgba(245,158,11,.25);background:rgba(245,158,11,.06);
      font-size:${size.xs};color:rgba(245,158,11,.9);line-height:1.5;
    }
    .${L}-precision-warn i{margin-right:3px}
    .${L}-ia{padding:0 10px 8px;flex:1 1 auto;min-height:0;display:flex;flex-direction:column}
    .${L}-panel.${L}-panel-mode-panel .${L}-ia{
      flex-shrink:0;flex-grow:0;min-height:auto;
    }
    .${L}-ir{display:flex;gap:6px;align-items:stretch;flex:1 1 auto;min-height:0}
    .${L}-panel.${L}-panel-mode-panel .${L}-ir{
    ...
    .${L}-panel.${L}-panel-mode-panel .${L}-in{
    ...
    .${L}-hn{padding:0 10px 6px;font-size:${size.xs};color:${C.textDim};display:flex;gap:10px;flex-shrink:0}
    ...
    .${L}-ei-multi{padding:6px 10px 4px;max-height:none;overflow:visible;display:flex;flex-direction:column;gap:6px}
    .${L}-ei-item{
      font-family:${mono};font-size:${size.sm};color:${C.hl};
      padding:8px 8px 6px;display:flex;align-items:flex-start;gap:6px;
      background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);
      border-radius:${radius.md};
    }
    .${L}-ei-item-primary{border-color:rgba(96,165,250,.25);background:rgba(96,165,250,.04)}
    .${L}-ei-rm{
      background:none;border:none;color:${C.textDim};cursor:pointer;
      font-size:${size.md};padding:1px 2px 0;font-family:'lucide'!important;font-style:normal;
      -webkit-font-smoothing:antialiased;transition:color .12s;
      flex-shrink:0;
    }
    .${L}-ei-rm:hover{color:${C.error}}
    .${L}-ei-item-body{min-width:0;flex:1}
    .${L}-ei-item-title{
      font-family:${mono};font-size:${size.sm};color:${C.hl};
      display:flex;align-items:center;gap:6px;white-space:nowrap;
    }
    .${L}-ei-item-tag{overflow:hidden;text-overflow:ellipsis;min-width:0;flex:1}
    .${L}-ei-item-focus{
      display:inline-flex;align-items:center;justify-content:center;
      padding:1px 5px;border-radius:${radius.full};
      border:1px solid rgba(96,165,250,.24);background:rgba(96,165,250,.12);
      color:${C.accent};font-size:${size.xs};font-weight:700;text-transform:uppercase;
      letter-spacing:.04em;flex-shrink:0;
    }
    .${L}-ei-item-sub{
      font-size:${size.sm};color:${C.textMuted};margin-top:2px;
      white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
    }
    .${L}-ei-item-path{
      font-size:${size.xs};color:${C.textDim};margin-top:2px;font-family:${mono};
      white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
    }

    /* ---- History panel ---- */
    #${L}-history{
      display:none;width:640px;height:400px;overflow-y:auto;
      border-top:1px solid ${C.panelBorder};
      border-radius:${radius.xl} ${radius.xl} 0 0;
    }
    #${L}-history.${L}-history-mode-panel{
    ...
    .${L}-hh{
      ...
      text-transform:uppercase;letter-spacing:.06em;color:${C.textMuted};cursor:grab;
    }
    .${L}-hh:active{cursor:grabbing}
    .${L}-hh-title{display:flex;align-items:center;gap:5px}
    .${L}-hh-title i{font-size:${size.base}}
    .${L}-hh-close{
      width:22px;height:22px;border-radius:${radius.md};border:none;background:transparent;
      color:${C.textDim};cursor:pointer;display:flex;align-items:center;justify-content:center;
      font-size:${size.lg};transition:${transition};margin-left:auto;
    ...
    .${L}-he-list{margin:6px 10px 10px;background:${C.surface};border:1px solid ${C.border};border-radius:${radius.lg};overflow:hidden;display:flex;flex-direction:column}
    .${L}-he{padding:8px 10px;border-bottom:1px solid ${C.border};font-size:${size.base};color:${C.text};display:flex;align-items:center;gap:8px;width:100%;box-sizing:border-box}
    ...
    .${L}-he-tag.manual{background:rgba(99,102,241,.12);color:rgba(129,140,248,.9)}
