...
export function barExpand(bar: HTMLElement, panel: HTMLElement) {
  ...
  if (
    collapsedRect.width < 1 ||
    expandedRect.width < 1 ||
    Math.abs(collapsedRect.width - expandedRect.width) < 1
  ) {
    return;
  }
  ...
  try {
    ...
    activeBarAnim.then(() => {
      ...
      // Check and clamp position only if overflow is detected after expansion
      requestAnimationFrame(() => {
        ...
        if (overflowX > 0 || overflowY > 0) {
          ...
          const newLeft = currentLeft - Math.max(0, overflowX);
          const newTop = currentTop - Math.max(0, overflowY);
          bar.style.left = `${Math.max(4, newLeft)}px`;
          bar.style.top = `${Math.max(4, newTop)}px`;
        }
      });
    });

    // Content fades in once bar is ~40% expanded
    activeContentAnim = animate(
      panel,
      {
        opacity: [0, 1],
        y: [6, 0],
...
export function toastOut(el: HTMLElement) {
  return animate(
    el,
    { opacity: [1, 0], x: [0, 30], scale: [1, 0.95] },
    { duration: 0.25 },
  );
}
