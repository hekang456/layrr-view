...
import { escapeHtml, toast } from './elements';
...
export async function fetchAndRenderHistory() {
  ...
  try {
    ...
    if (data.commits.length === 0) {
      container.innerHTML = `<div class="${L}-hh"><span class="${L}-hh-title"><i class="icon-grip-vertical"></i><span>History</span></span><button class="${L}-hh-close"><i class="icon-x"></i></button></div><div class="${L}-he-empty">No edits yet</div>`;
      container
        .querySelector(`.${L}-hh-close`)
    ...
    const isPreviewingOlder =
      app.previewingHash && app.previewingHash !== latestHash;

    let html = `<div class="${L}-hh"><span class="${L}-hh-title"><i class="icon-grip-vertical"></i><span>History (${data.commits.length})</span></span>${nav}<button class="${L}-hh-close"><i class="icon-x"></i></button></div>`;
    ...
    html +=
      page
        .map((c, i) => {
          ...
          const isActive = app.previewingHash
            ? c.hash === app.previewingHash
            : c.hash === data.head && i === 0;
          const clickable = !isActive;
          const cls =
            (isActive ? 'active' : '') + (clickable ? ' clickable' : '');

          let tag = '';
          if (isLatest) tag = `<span class="${L}-he-tag latest">latest</span>`;
          else if (!c.isPair)
            tag = `<span class="${L}-he-tag manual">git</span>`;
          ...
          return (
            `<div class="${L}-he ${cls}" data-hash="${c.hash}">` +
            `<div class="${L}-he-body">` +
            `<div class="${L}-he-inst" title="${escapeHtml(c.message)}">${escapeHtml(c.message)}${tag}</div>` +
            `<div class="${L}-he-el">${escapeHtml(c.timeAgo)}</div>` +
            `</div>` +
    ...
    // Wire up events
    container.querySelector(`.${L}-hh-prev`)?.addEventListener('click', () => {
      app.historyPage--;
      fetchAndRenderHistory();
    });
    container.querySelector(`.${L}-hh-next`)?.addEventListener('click', () => {
      app.historyPage++;
      fetchAndRenderHistory();
    });
    container
    ...
    container.innerHTML = `<div class="${L}-hh"><span class="${L}-hh-title"><i class="icon-grip-vertical"></i><span>History</span></span><button class="${L}-hh-close"><i class="icon-x"></i></button></div><div class="${L}-he-empty">Could not load history</div>`;
    container
      .querySelector(`.${L}-hh-close`)
      ?.addEventListener('click', () => closeHistory());
  }
